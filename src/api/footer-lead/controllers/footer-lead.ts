...
async function resolvePageData(pageSlug: string): Promise<{
  ...
  const defaults = {
    pageUrl: BASE_URL,
    formTitle: 'New footer Lead',
    emailSubject: 'New footer Lead | Korcomptenz',
    emailBody: '',
  };
      ...
      list: {
        on: {
          ...
          'home.schedule-call': {
            populate: {
              list: {
                select: ['formTitle', 'emailSubject', 'emailBody' , 'adminEmailSubject' , 'adminEmailBody'],
              },
            },
          },
        },
...
export default factories.createCoreController('api::footer-lead.footer-lead', ({ strapi }) => ({
  async create(ctx) {
    try {
        ...
        <td style="padding:10px 20px 20px;font-family:Arial,sans-serif;">
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
            ...
            <tr>
              ...
              <td style="padding:10px;border-bottom:1px solid #CCC;text-align:left;">${data?.Name || '-'}</td>
            </tr>

            <tr style="background:#f4f5f7;">
              <td style="padding:10px;font-weight:600;border-bottom:1px solid #CCC;text-align:left;">Email</td>
              <td style="padding:10px;border-bottom:1px solid #CCC;text-align:left;">${data?.Email || '-'}</td>
            </tr>
