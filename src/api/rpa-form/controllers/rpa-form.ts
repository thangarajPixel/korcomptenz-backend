...
async function resolvePageData(pageSlug: string): Promise<{
  ...
  const defaults = {
    pageUrl: BASE_URL,
    formTitle: 'New RPA Lead',
    emailSubject: 'New RPA Lead | Korcomptenz',
    emailBody: '',
  };
...
export default factories.createCoreController('api::rpa-form.rpa-form', ({ strapi }) => ({
  async create(ctx) {
    try {
...
<html lang="en">
  ...
  <body>
    <table border="0" width="600" cellpadding="0" cellspacing="0"
      ...
      console.error('RPA lead error:', error);
